...
if [[ "${SIGNER_MODE:-keystore}" == "keystore" ]]; then
    # Keep stderr attached to the terminal so the no-echo keystore password
    # prompt remains visible. Signer diagnostics never contain key material.
    INSTRUCTION_SENDER=$(go run ./cmd/deploy-contract -a "$ADDRESSES_FILE" -c "$CHAIN_URL" | tail -1) || {
        die "Deploy failed — see terminal output above"
    }
else
...
if [[ "${SIGNER_MODE:-keystore}" == "keystore" ]]; then
    EXTENSION_ID=$(go run ./cmd/register-extension -a "$ADDRESSES_FILE" -c "$CHAIN_URL" --instructionSender "$INSTRUCTION_SENDER" | tail -1) || {
        die "Registration failed — see terminal output above"
    }
else
