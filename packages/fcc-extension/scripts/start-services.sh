...
PROXY_SIGNER_MODE="${PROXY_SIGNER_MODE:-${SIGNER_MODE:-keystore}}"
case "$PROXY_SIGNER_MODE" in
    keystore|raw) ;;
    *) die "PROXY_SIGNER_MODE must be 'keystore' or 'raw'" ;;
esac
if [[ "$PROXY_SIGNER_MODE" == "raw" ]]; then
    PROXY_PRIVATE_KEY="${PROXY_PRIVATE_KEY:?Set PROXY_PRIVATE_KEY only in an untracked environment for raw mode}"
else
    if [[ -n "${PROXY_PRIVATE_KEY:-}" || -n "${PRIVATE_KEY:-}" ]]; then
        die "Raw PROXY_PRIVATE_KEY/PRIVATE_KEY must be unset when PROXY_SIGNER_MODE=keystore"
    fi
    unset PROXY_PRIVATE_KEY PRIVATE_KEY
    if [[ ! -t 0 || ! -t 1 || ! -t 2 ]]; then
        die "Interactive TTY required for proxy keystore password. Run this command directly in an Ubuntu WSL terminal without pipes or redirection."
    fi
fi
...
if [[ "$USE_LOCAL" == "false" ]]; then
    ...
    if [[ -z "${REGISTRY:-}" ]]; then
        if [[ "$PROXY_SIGNER_MODE" == "keystore" && "$USE_LOCAL_SIBLINGS" == "true" ]]; then
            die "Keystore proxy mode requires the pinned AVERLOCK proxy overlay; unset USE_LOCAL_SIBLINGS"
        fi
        if [[ "$PROXY_SIGNER_MODE" == "keystore" ]] || ! docker image inspect local/tee-proxy >/dev/null 2>&1; then
            if [[ "$USE_LOCAL_SIBLINGS" == "true" ]]; then
                ...
                docker build -f "$PROXY_DOCKERFILE" -t local/tee-proxy "$PROJECT_DIR" || die "Failed to build tee-proxy image"
            fi
    ...
    if [[ "$PROXY_SIGNER_MODE" == "keystore" ]]; then
        COMPOSE_FILES+=("-f" "$PROJECT_DIR/docker-compose.keystore.yaml")
    fi
    ...
    if [[ "$PROXY_SIGNER_MODE" == "keystore" ]]; then
        ...
        log "Keystore mode uses a deliberate two-terminal lifecycle."
        log "Terminal 1 owns the proxy password TTY; Terminal 2 starts the TEE only after health verification."
        exec "$SCRIPT_DIR/start-proxy.sh" --chain "$CHAIN"
    fi
...
# ============================================================
if [[ "$PROXY_SIGNER_MODE" == "keystore" ]]; then
    die "Encrypted proxy keystore mode is supported through Docker only; omit --local"
fi
