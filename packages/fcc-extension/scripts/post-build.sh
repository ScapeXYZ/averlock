...
# Auto-detect proxy ports only for local development. A live registration must
# receive an explicit permanent public HTTPS origin; its exact value is stored
# on-chain and is later checked fail-closed by the web API.
if [[ "${LOCAL_MODE:-true}" == "false" && -z "${EXT_PROXY_URL:-}" ]]; then
    die "EXT_PROXY_URL is required when LOCAL_MODE=false; use the permanent HTTPS proxy origin"
fi
if [[ "${LOCAL_MODE:-true}" == "false" && -z "${EXT_PROXY_HOST_URL:-}" ]]; then
    die "EXT_PROXY_HOST_URL is required when LOCAL_MODE=false; it must match the permanent registered HTTPS origin"
fi
