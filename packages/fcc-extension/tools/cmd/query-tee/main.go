...
func main() {
	...
	fsm := flag.String("fsm", "", "optional FlareSystemsManager address for current reward epoch")
	...
	if *fsm != "" {
		sm, bindErr := system.NewFlareSystemsManager(common.HexToAddress(*fsm), cc)
		if bindErr != nil {
			fmt.Printf("getCurrentRewardEpochId bind ERROR: %v\n", bindErr)
		} else if epoch, callErr := sm.GetCurrentRewardEpochId(opts); callErr != nil {
			fmt.Printf("getCurrentRewardEpochId ERROR: %v\n", callErr)
		} else {
			fmt.Printf("getCurrentRewardEpochId: %s\n", epoch.String())
		}
	}
