...
import (
	...
	"extension-scaffold/tools/pkg/contracts/averlock"
...
func DeployInstructionSender(s *support.Support) (common.Address, *averlock.AverlockInstructionSender, error) {
	...
	address, tx, contract, err := averlock.DeployAverlockInstructionSender(
...
func SetExtensionId(s *support.Support, instructionSenderAddress common.Address) error {
	sender, err := averlock.NewAverlockInstructionSender(instructionSenderAddress, s.ChainClient)
	if err != nil {
	...
	if err != nil {
		...
		if reason == "" {
			parsed, _ := averlock.AverlockInstructionSenderMetaData.GetAbi()
			if parsed != nil {
...
func SendCreatePolicy(s *support.Support, instructionSenderAddress common.Address, encryptedPolicy []byte) (common.Hash, common.Hash, error) {
	sender, err := averlock.NewAverlockInstructionSender(instructionSenderAddress, s.ChainClient)
	if err != nil {
	...
	tx, err := sender.CreatePolicy(opts, encryptedPolicy)
	if err != nil {
		reason := fccutils.DecodeRevertReason(err)
		if reason == "" {
			parsed, _ := averlock.AverlockInstructionSenderMetaData.GetAbi()
			if parsed != nil {
				callData, packErr := parsed.Pack("createPolicy", encryptedPolicy)
				if packErr == nil {
					from := crypto.PubkeyToAddress(s.Prv.PublicKey)
					reason = fccutils.SimulateAndDecodeRevert(
...
func SendEvaluateGuard(s *support.Support, instructionSenderAddress common.Address, evaluationContext []byte) (common.Hash, common.Hash, error) {
	sender, err := averlock.NewAverlockInstructionSender(instructionSenderAddress, s.ChainClient)
	if err != nil {
		return common.Hash{}, common.Hash{}, errors.Errorf("failed to bind contract: %s", err)
	}
	...
	tx, err := sender.EvaluateGuard(opts, evaluationContext)
	if err != nil {
		return common.Hash{}, common.Hash{}, errors.Errorf("failed to send instruction: %s", err)
	}
