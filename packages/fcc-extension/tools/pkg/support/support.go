...
func DefaultSupport(AddressesFilePath, chainNodeURL string) (*Support, error) {
	...
	if err != nil {
		return nil, errors.Errorf("querying chain ID: %s", err)
	}
	// Lifecycle commands run from tools/, while the untracked configuration
	// belongs at the scaffold root. The shell scripts normally source it first,
	// but direct Go invocations should resolve the same file without producing a
	// misleading "tools/.env missing" warning.
	for _, envPath := range []string{".env", "../.env"} {
		if _, statErr := os.Stat(envPath); statErr == nil {
			if loadErr := godotenv.Load(envPath); loadErr != nil {
				return nil, errors.Errorf("loading environment file %s: %s", envPath, loadErr)
			}
			break
		}
	}
	privKey, address, err := signer.LoadFromEnvironment(chainID)
	if err != nil {
		return nil, err
	}
	fmt.Fprintf(os.Stderr, "Signer initialized: mode=%s address=%s chain_id=%s\n", os.Getenv("SIGNER_MODE"), address.Hex(), chainID.String())
