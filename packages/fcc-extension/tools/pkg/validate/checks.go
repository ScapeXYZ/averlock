...
func CheckDeployerKeySource() CheckResult {
	if strings.EqualFold(strings.TrimSpace(os.Getenv("SIGNER_MODE")), "keystore") ||
		(os.Getenv("SIGNER_MODE") == "" && os.Getenv("DEPLOYMENT_KEYSTORE_ACCOUNT") != "") {
		return CheckResult{
			Step: "deploy", ID: "D5", Name: "deployer key source", Status: PASS,
			Message: "encrypted keystore signer configured",
		}
	}
...
func RegisterServicesChecks(r *Report, extensionEnvPath string) {
	...
	// S10: proxy signer configured on non-local.
	localMode := os.Getenv("LOCAL_MODE")
	privateKey := os.Getenv("PROXY_PRIVATE_KEY")
	keystoreMode := strings.EqualFold(strings.TrimSpace(os.Getenv("PROXY_SIGNER_MODE")), "keystore")
	if localMode == "true" || localMode == "" {
		r.Add(CheckResult{
			Step:    "services",
			ID:      "S10",
	...
	} else if keystoreMode && strings.TrimSpace(os.Getenv("PROXY_KEYSTORE_PATH")) != "" {
		r.Add(CheckResult{
			Step: "services", ID: "S10", Name: "proxy signer configured", Status: PASS,
			Message: "encrypted proxy keystore configured",
		})
...
func RegisterTeeVersionChecks(r *Report, extensionEnvPath string) {
	...
	privKey := os.Getenv("DEPLOYMENT_PRIVATE_KEY")
	localMode := os.Getenv("LOCAL_MODE")
	keystoreMode := strings.EqualFold(strings.TrimSpace(os.Getenv("SIGNER_MODE")), "keystore")

	if keystoreMode {
		r.Add(CheckResult{
			Step: "tee-version", ID: "V2", Name: "extension owner key", Status: PASS,
			Message: "using encrypted deployment keystore",
		})
	} else if extOwnerKey != "" {
		r.Add(CheckResult{
			Step:    "tee-version",
